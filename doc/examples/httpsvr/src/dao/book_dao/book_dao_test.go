...
func TestBookDao(t *testing.T) {
	...
	assert.ThatString(t, x.SvrAddr).Equal("0.0.0.0:9090")
	...
	assert.Nil(t, err)
	assert.That(t, len(books)).Equal(0)
	...
	assert.That(t, books[0].SN).Equal("1")
	assert.That(t, books[0].Name).Equal("Go Spring")
